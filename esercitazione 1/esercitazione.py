...
def check_int_str(stringa, intero):
    return stringa.count(str(intero))
...
def reshape_by_index(L, src, dst):
    result = [ None for i in range(max(dst)+1)]
    for i in range(len(src)):
        result[dst[i]] = L[src[i]]
    return result
...
def sort_by_str(T):
    ...
    def rules(el):
        return -len(el), el
    result = sorted(T, key=rules, reverse=True)
    return tuple(result)
...
def sort_by_month(L):
    ...
    for mese in mesi:
        if mese in L:
            result.append(mese)
        elif mese.capitalize() in L:
            result.append(mese.capitalize())
    return result
